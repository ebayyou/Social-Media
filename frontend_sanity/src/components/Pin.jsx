  ...
  return (
    <div className="m-2">
          ...
          <div className="absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50" style={{ height: '100%' }}>
            ...
            <div className=" flex justify-between items-center gap-2 w-full">
              ...
              {postedBy?._id === user?.sub && (
                <button type="button" className="bg-white p-2 rounded-full w-8 h-8 flex items-center justify-center text-dark opacity-75 hover:opacity-100 outline-none"
                  onClick={(e) => {
                    e.stopPropagation();
                    deletePin(_id)
                  }}
      ...
      <Link to={`/user-profile/${postedBy?._id}`} className="flex gap-2 mt-2 items-center">
        <img src={postedBy?.image} alt={postedBy?.userName} className="w-8 h-8 rounded-full object-cover" />
